...
@testset "attach" begin
    ...
    @test  @attach(df, :A .+ 1)   ==  df.A .+ 1
    @test  @attach(df, :A .+ :B)  ==  df.A .+ df.B
    @test  @attach(df, :A .+ x)   ==  df.A .+ x
    ...
    idx = :A
    @test  @attach(df, $idx .+ :B)  ==  df.A .+ df.B
    idx2 = :B
    @test  @attach(df, $idx .+ $idx2)  ==  df.A .+ df.B
    @test  @attach(df, $:A .+ $"B")  ==  df.A .+ df.B

    @test_throws ArgumentError @attach(df, :A + $2)

    @test  x == sum(df.A .* df.B)
    @test  @attach(df, df[:A .> 1, ^([:B, :A])]) == df[df.A .> 1, [:B, :A]]
    @test  @attach(df, DataFrame(a = :A * 2, b = :A .+ :B)) == DataFrame(a = df.A * 2, b = df.A .+ df.B)

    @test @attach(df, :A) === df.A
    @test @attach(df, $:A) === df.A
    @test @attach(df, $"A") === df.A
end
